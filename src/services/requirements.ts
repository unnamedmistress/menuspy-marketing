...
export async function getRequirementsForJob(
  ...
  if (cached) {
    ...
    return cached.map((r) => normalizeRequirement({ ...r, id: `req-${Date.now()}-${Math.random()}` }));
  }
  ...
  try {
    ...
    const cacheableRequirements = analysis.requirements.map((r) => normalizeRequirement({
      ...r,
    ...
    return analysis.requirements.map(normalizeRequirement);
  } catch (error) {
    console.error('Failed to get requirements:', error);
    return getDefaultRequirements(jobType).map(normalizeRequirement);
  }
}
...
function normalizeRequirement(requirement: Requirement): Requirement {
  return {
    ...
    minimumCriteria: requirement.minimumCriteria || 'Names, address, and signatures are clear',
